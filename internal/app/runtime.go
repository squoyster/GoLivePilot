...
// initStreamSwitch creates a StreamSwitch from pipeline bus node and targets config.
func (r *Runtime) initStreamSwitch(cfg *config.Config) {
	...
	for _, node := range cfg.Pipeline.Nodes {
		...
		// Build outputs from enabled targets that publish to RTMP/RTMPS
		for _, t := range cfg.Targets {
			if !t.Enabled {
				continue
			}
			url := resolveTargetURL(&t)
			if url == "" {
				continue
			}
			var encodeArgs []string
			if strings.Contains(t.Platform, "facebook") || strings.Contains(strings.ToLower(t.Label), "facebook") {
...
// resolveTargetURL resolves the full RTMP/RTMPS URL for a target config.
func resolveTargetURL(t *config.TargetConfig) string {
	url := os.Getenv(t.RTMPSURLEnv)
	if url == "" {
