...
internal/config/              # YAML config loading (single file or directory)
internal/ffmpeg/              # FFmpeg process supervisor
internal/mediamtx/            # MediaMTX API client
internal/server/              # HTTP handlers and routing
internal/streamswitch/        # Go-based RTMP stream switcher (program bus)
configs/                      # Config directories (only example/ is tracked)
...
- Run Go natively: `go run ./cmd/golivepilot --config-dir configs/local`
- Run MediaMTX in Docker separately
- Create local config: `cp -r configs/example configs/local`
- Default listen address: `:3000`

## Config & Secrets
- **Config**: Single YAML file (`--config`) or directory of YAML files (`--config-dir`)
- **Directory mode**: All `*.yml` files in the directory are loaded and merged alphabetically
- **Only `configs/example/` is tracked in git** — all other config dirs are gitignored
- **Secrets**: Environment variables only (never in browser localStorage or YAML files)
